...
<template>
  ...
  >
    <div class="mb-4 flex items-start justify-between max-[600px]:flex-col max-[600px]:gap-2">
      <div class="flex items-center gap-4 max-[600px]:flex-col max-[600px]:items-start max-[600px]:gap-2">
        <h3 class="text-[1.1rem] font-bold text-brand-navy">{{ submission.name }}</h3>
        <BadgeDropdown
      ...
      <span class="whitespace-nowrap text-[0.8rem] text-brand-steel max-[600px]:text-sm">{{ formatDate(submission.created_at) }}</span>
    </div>

    <div class="grid gap-[0.65rem] text-brand-steel">
      <p><strong>Email:</strong> {{ submission.email }}</p>
      <p v-if="submission.phone"><strong>Phone:</strong> {{ submission.phone }}</p>
      <p class="mt-[0.35rem] whitespace-pre-line rounded-[0.6rem] border border-[#e2e9f5] bg-[#f5f8fd] px-4 py-[0.85rem]">
