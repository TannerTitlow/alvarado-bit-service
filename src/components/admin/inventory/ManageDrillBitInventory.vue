...
const inventoryMetrics = computed(() => [
  ...
  },
  {
    ...
    detail: 'held for a job or customer',
  },
  {
...
const modelMetrics = computed(() => [
  {
    ...
    detail: 'active drill-bit configurations',
  },
  {
...
<template>
  <section class="p-[clamp(1rem,2.5vw,2rem)] text-admin-ink max-[640px]:p-4">
    <header
      class="flex items-center justify-between gap-6 rounded-[1rem] border border-admin-border bg-workspace-header p-[clamp(1.25rem,3vw,2rem)] shadow-panel max-[640px]:flex-col max-[640px]:items-stretch"
    >
      <div>
        <p
          class="mb-[0.3rem] mt-0 font-secondary text-[0.7rem] font-bold uppercase tracking-[0.11em] text-admin-eyebrow"
        >
          Operations inventory
        </p>
        <h2 class="m-0 text-[clamp(1.45rem,2.6vw,2rem)] text-brand-navy">
          Drill Bit Inventory
        </h2>
        <p class="mb-0 mt-[0.4rem] text-admin-muted">
      ...
      <BaseButton
        variant="danger"
        class="max-[640px]:min-h-[2.9rem] max-[640px]:w-full"
        @click="
          currentTab === 'models'
            ? openModelModal()
    ...
    <nav
      class="mt-5 flex gap-[0.35rem] rounded-toolbar border border-admin-border bg-admin-panel-tab p-[0.35rem]"
      aria-label="Inventory sections"
    >
      <button
        :class="[
          'flex-1 cursor-pointer rounded-control border-0 px-4 py-3 font-bold text-admin-muted transition-[background-color,color,box-shadow] duration-interaction focus-visible:outline-none focus-visible:shadow-focus-strong max-[640px]:px-[0.45rem] max-[640px]:py-[0.65rem] max-[640px]:text-[0.82rem] motion-reduce:transition-none',
          currentTab === 'inventory'
            ? 'bg-admin-panel text-brand-navy shadow-tab-active'
            : 'bg-transparent',
        ]"
        type="button"
      ...
      >
        Physical Inventory
        <span
          class="ml-[0.35rem] inline-block rounded-full bg-admin-panel-count px-[0.36rem] py-[0.08rem] text-[0.75rem]"
          >{{ units.length }}</span
        >
      </button>
      <button
    ...
    <p
      v-if="notice"
      class="my-4 rounded-control bg-admin-notice-soft px-4 py-3 text-admin-notice"
    >
      {{ notice }}
    </p>
    <p
      v-if="errorMessage"
      class="my-4 rounded-control bg-admin-danger-soft px-4 py-3 text-admin-danger"
    >
      {{ errorMessage }}
    </p>
    <InventoryToolbar
      ...
      @update:unit-search="unitSearch = $event"
      @update:status="status = $event"
      @update:attention-only="attentionOnly = $event"
    />
    ...
    >
      <div
        v-if="loading"
        key="loading"
        class="m-0 px-4 py-10 text-center text-admin-muted"
      >
      ...
      <div
        ...
        key="models"
        class="mt-5 grid grid-cols-[repeat(auto-fill,minmax(22rem,1fr))] gap-4 max-[640px]:grid-cols-1"
      >
        <p
          v-if="!filteredModels.length"
          class="col-span-full m-0 px-4 py-10 text-center text-admin-muted"
        >
        ...
        <DrillBitModelCard
          ...
          :key="model.id"
          :model="model"
          :available="availableFor(model.id)"
          :unit-count="unitsFor(model.id).length"
          :price="money(model.list_price)"
          @edit="openModelModal"
          @delete="deletingModel = $event"
        />
      </div>
      <InventoryUnitTable
        v-else
